...
import { X, Smartphone, CreditCard, ShieldCheck, Loader2, Wallet } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col">
        ...
        {/* Header */}
        <div className="bg-gray-50 p-4 flex justify-between items-center border-b border-gray-100 shrink-0">
        ...
        <div className="p-6 overflow-y-auto">
          {step === 'SELECT' && (
            <>
              ...
              <div className="mb-6">
                <h4 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wide">Moyen de paiement</h4>
                <div className="space-y-3">
                  ...
                  >
                    ...
                    {method === 'MOBILE' && <div className="w-4 h-4 rounded-full bg-orange-500 border-2 border-white shadow-sm"></div>}
                  </button>

                  {/* Credit Card */}
                  <button 
                    onClick={() => setMethod('CARD')}
                    className={`w-full flex items-center justify-between p-4 rounded-xl border-2 transition ${method === 'CARD' ? 'border-purple-500 bg-purple-50' : 'border-gray-100 hover:border-gray-200'}`}
                  >
                    <div className="flex items-center">
                    ...
                    {method === 'CARD' && <div className="w-4 h-4 rounded-full bg-purple-500 border-2 border-white shadow-sm"></div>}
                  </button>

                  {/* PayPal */}
                  <button 
                    onClick={() => setMethod('PAYPAL')}
                    className={`w-full flex items-center justify-between p-4 rounded-xl border-2 transition ${method === 'PAYPAL' ? 'border-blue-600 bg-blue-50' : 'border-gray-100 hover:border-gray-200'}`}
                  >
                    <div className="flex items-center">
                    ...
                    {method === 'PAYPAL' && <div className="w-4 h-4 rounded-full bg-blue-600 border-2 border-white shadow-sm"></div>}
                  </button>
                </div>
              </div>
              ...
              <button 
                onClick={handlePay}
                className="w-full py-3 bg-primary hover:bg-teal-700 text-white font-bold rounded-xl shadow-lg transition transform active:scale-[0.99]"
              >
          ...
          {step === 'PROCESS' && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-sm text-gray-400 mt-2">
                {method === 'MOBILE' ? 'Veuillez valider sur votre téléphone.' : method === 'PAYPAL' ? 'Redirection vers PayPal...' : 'Validation bancaire en cours...'}
              </p>
            </div>
          )}
